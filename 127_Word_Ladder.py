...
class Solution1(object):
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        while queue:
            ...
            successors = self.findSuccessors(word, wordList)
            #print(successors)
            for s in successors:
                if s not in visited and s not in [q[0] for q in queue]:
                    queue.append((s,level+1))
        return(0)
        
    def findSuccessors(self, word, wordlist):
        n = len(word)
        res = []
        for i in range(n):
            new_word = word[0:i]+word[i+1:]
            new_wordlist = map(lambda x: x[0:i]+x[i+1:], wordlist)
            for i in range(len(new_wordlist)):
                if new_wordlist[i] == new_word and wordlist[i] != word:
...
class Solution(object):
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        :rtype: int
        """
        wordhash = self.construct_dict(wordList)
        queue = [(beginWord,1)]
        #if use visited as arr, time limit exceed, use set will quick
        #visited = []
        visited = {}
        while queue:
            #print(visited)
            ...
            for i in range(len(word)):
                ...
                successors = wordhash.get(new_word,[])
                for s in successors:
                    if s not in visited :
                        queue.append((s,level+1))
        return(0)
...
class Solution:
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        #graph + bfs (dijistra for weight graph)
        ...
        while queue:
            ...
            for i in range(l):
                for j in "abcdefghijklmnopqrstuvwxyz":
                    ...
                    if tmp in wordSet and tmp not in visit:
